...
Matrix time(Matrix a, Matrix b) {
    ...
    qsort(ret.m + 1, ret.m[0].v, sizeof(element), cmp);
...
Matrix reverse(Matrix a) {
    ...
    row_terms = calloc(a.m[0].c, sizeof(int));
    // memset(row_terms, 0, a.m[0].c);
    startposition = calloc(a.m[0].c, sizeof(int));
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    if (!strcmp(arr[i].name, name1)) {
                        for (int j = 0; j < length; j++) {
                            // printf("%d %s %s\n", j, arr[j].name, name2);
                            if (!strcmp(arr[j].name, name2)) {
                                exist = 1;
                                printMartix(merge(arr[i], arr[j]));
                                break;
                            }
                        }
                    }
